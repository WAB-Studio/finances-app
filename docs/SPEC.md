...
- [ ] **RF-127** — The transaction listing also filters by the recurring rule that generated a movement, alongside the RF-23 and RF-89 filters; a rule opens that filtered listing from its own row, and the filtered export (RF-118) carries the same narrowing.
